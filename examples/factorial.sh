...
DECLARATIONS
    ...
    Need: (Integer) -> Boolean;	# предикат Need задаёт числа, для которых нужно считать факториал
	Fact: (Integer) -> Integer;	# функция Fact возвращает факториал числа
	
FACTS

    Need(6);		# ищем факториал шести
	Fact(1) = 1;	# начальное значение факториала

    # распространяем предикат Need на предыдущие числа
	All n : Integer
	    Need(n) & n > 2 => Need(n - 1);

    # вычисляем значение факториала
	All n : Integer
	    Need(n) => Fact(n) = (n * Fact(n - 1));
